...
describe('Given two authenticated users', () => {
  ...
  describe('When user A sends a DM to user B', () => {
    ...
    beforeAll(async () => {
      conversation = await when.we_invoke_sendDirectMessage(
        userA.username,
        userB.username,
        text
      );
    ...
    it('Saves the message in the DirectMessages table', async () => {
      const [message] = await then.there_are_N_messages_in_DirectMessagesTable(
        conversation.id,
        1
      );
    ...
    it('Saves the conversation in the Conversations table for both user A and user B', async () => {
      await then.conversation_exists_in_ConversationsTable(
        userA.username,
        userB.username
      );
      await then.conversation_exists_in_ConversationsTable(
